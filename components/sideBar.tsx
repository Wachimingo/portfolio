...
const SideBar = ({ nav, project }: SideBarProps) => {
    console.log("TCL: SideBar -> nav", nav)
    const [hide, setHide] = useState(false);
    const { session }: any = useContext(AuthContext);
    return (
        // <Suspense fallback={<>Loading</>}>
        <>
            <div className={`${hide ? 'hidden' : ''} fixed top-16 flex flex-col w-56 bg-white rounded-r-3xl overflow-hidden`}>
                <div className="flex items-center justify-center h-20 shadow-md">
                    <h1 className="text-3xl uppercase text-indigo-500">{project}</h1>
                </div>
                <ul className="flex flex-col py-4">
                    {
                        nav.map((navItem: any, i: number) => {
            ...
            <button
                type="button"
                className={`fixed top-24 ${hide ? 'left-0' : 'left-52'} bg-white rounded-full flex text-black`}
                style={{ fontSize: "2vw" }}
